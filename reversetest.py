...
Capabilities:
  ...
  6. Perform a joint SN+DESI grid scan:
       (a) over (H0, Omega_m) for a fixed 3-phase k_eff(a) pattern, and
       (b) over the full 7D parameter space
           (H0, Omega_m, k1, k2, k3, a1, a2)
     and report the best-fit combined point, plus likelihood ratio vs
     a ΛCDM-like logistic baseline.
"""
...
# Full 7D joint scan settings (can be tuned)
JOINT_K1_RANGE = SCAN_K1_RANGE
JOINT_K2_RANGE = SCAN_K2_RANGE
JOINT_K3_RANGE = SCAN_K3_RANGE
JOINT_A1_RANGE = SCAN_A1_RANGE
JOINT_A2_RANGE = SCAN_A2_RANGE
JOINT_N_K_PER_DIM = SCAN_N_PER_DIM   # coarse global search in k1,k2,k3,a1,a2
...
# ============================================================
# SN DISTANCES & χ² FOR LOGISTIC BACKGROUND (ΛCDM-LIKE)
# ============================================================

def build_background_for_distances_logistic(H0, Omega_m, k_max):
    """
    Build a background H(a) and comoving distance integral
    for the logistic k_eff(a) model.
    ...
    if Omega_BH0 <= 0:
        raise ValueError(
            f"[logistic] Omega_BH0 <= 0 for H0={H0}, Omega_m={Omega_m}. "
            f"Omega_r={Omega_r}, Omega_BH0={Omega_BH0}"
        )
    ...
    Omega_BH_vals = rho_BH_of_a_logistic(
        a_grid_desc, Omega_BH0, k_max=k_max, a_t=A_TRANS, gamma=GAMMA_KEFF
    )
...
def sn_chi2_logistic(H0, Omega_m, k_max, z_sn, mu_sn, mu_err_sn):
    """
    Compute Pantheon+ chi^2 for a given (H0, Omega_m, k_max)
    using the logistic k_eff(a) background. We analytically minimize
    over an additive offset M.
    """
    a_grid_desc, Dint_desc = build_background_for_distances_logistic(
        H0, Omega_m, k_max
    )
...
# ============================================================
# SN DISTANCES & χ² FOR 3-PHASE PIECEWISE BACKGROUND
# ============================================================
...
# ============================================================
# JOINT SN+DESI SCAN OVER (H0, Omega_m) FOR FIXED k_i, a_i
# ============================================================
...
# ============================================================
# FULL 7-PARAMETER JOINT SN+DESI SCAN
# ============================================================

def joint_scan_all_params(
    z_sn, mu_sn, mu_err_sn,
    desi_mean, desi_cov,
    H0_range=JOINT_H0_RANGE,
    Om_range=JOINT_OM_RANGE,
    k1_range=JOINT_K1_RANGE,
    k2_range=JOINT_K2_RANGE,
    k3_range=JOINT_K3_RANGE,
    a1_range=JOINT_A1_RANGE,
    a2_range=JOINT_A2_RANGE,
    n_H0=JOINT_N_H0,
    n_Om=JOINT_N_OM,
    n_kdim=JOINT_N_K_PER_DIM
):
    """
    Full nested grid scan over the 7 parameters:
      (H0, Omega_m, k1, k2, k3, a1, a2)
    to find the global minimum of chi^2_SN+DESI
    within the coarse prior box.

    Returns:
      best = (
        H0_best, Om_best,
        k1_best, k2_best, k3_best, a1_best, a2_best,
        M_best_SN, H0rs_best,
        chi2_SN_best, chi2_DESI_best, chi2_tot_best
      )
      or None if no viable model passes the early-time safety cuts.
    """
    H0_vals = np.linspace(H0_range[0], H0_range[1], n_H0)
    Om_vals = np.linspace(Om_range[0], Om_range[1], n_Om)

    k1_vals = np.linspace(k1_range[0], k1_range[1], n_kdim)
    k2_vals = np.linspace(k2_range[0], k2_range[1], n_kdim)
    k3_vals = np.linspace(k3_range[0], k3_range[1], n_kdim)
    a1_vals = np.linspace(a1_range[0], a1_range[1], n_kdim)
    a2_vals = np.linspace(a2_range[0], a2_range[1], n_kdim)
                    ...
                    for a2 in a2_vals:
                        ...
                        for H0 in H0_vals:
                            for Om in Om_vals:
                                ...
                                # SN chi^2
                                try:
                                    chi2_SN, M_best = sn_chi2_piecewise(
                                        H0, Om, k1, k2, k3, a1, a2,
                                        z_sn, mu_sn, mu_err_sn
                                    )
                                except Exception:
                                    continue

                                # DESI chi^2 in (H0*r_s, Omega_m) space
                                try:
                                    H0rs, Om_eff = compute_H0rs_emu_piecewise(
                                        H0, Om, k1, k2, k3, a1, a2
                                    )
                                except Exception:
                                    continue
...
def main():
    ...
    print("=== Step 5: Load Pantheon+ and run joint SN+DESI scans ===")
    z_sn, mu_sn, mu_err_sn = load_pantheon_subset(PANTHEON_FILE, zmin=0.01, zmax=None)
    print(f"Loaded {len(z_sn)} SNe for joint fit")
    print()

    # 5a. ΛCDM-like logistic baseline joint χ^2 (SN+DESI)
    print("Computing ΛCDM-like logistic joint SN+DESI baseline:")
    try:
        chi2_SN_LCDM, M_LCDM = sn_chi2_logistic(
            LCDM_H0, LCDM_OM, k_max=3.0, z_sn=z_sn, mu_sn=mu_sn, mu_err_sn=mu_err_sn
        )
        chi2_DESI_LCDM = dchi2_lcdm  # by construction: DESI χ^2 in (H0*r_s, Omega_m) space
        chi2_tot_LCDM = chi2_SN_LCDM + chi2_DESI_LCDM
        print(f"  SN chi^2 (ΛCDM-like logistic)   = {chi2_SN_LCDM:.2f}")
        print(f"  DESI chi^2 (ΛCDM-like logistic) = {chi2_DESI_LCDM:.2f}")
        print(f"  Total chi^2 (SN+DESI)           = {chi2_tot_LCDM:.2f}")
        print(f"  Best-fit SN offset M (ΛCDM-like)= {M_LCDM:.4f}")
    except Exception:
        chi2_SN_LCDM = None
        chi2_tot_LCDM = None
        print("  WARNING: Could not compute SN chi^2 for ΛCDM-like logistic baseline.")
    print()

    # 5b. Fixed-pattern 2D joint scan in (H0, Omega_m)
    ...
    # 5c. Full 7-parameter joint scan (H0, Omega_m, k1, k2, k3, a1, a2)
    print("=== Step 6: Full 7-parameter joint SN+DESI scan ===")
    print("Scanning over:")
    print("  (H0, Omega_m, k1, k2, k3, a1, a2)")
    print(f"  H0 in [{JOINT_H0_RANGE[0]}, {JOINT_H0_RANGE[1]}] with {JOINT_N_H0} steps")
    print(f"  Omega_m in [{JOINT_OM_RANGE[0]}, {JOINT_OM_RANGE[1]}] with {JOINT_N_OM} steps")
    print(f"  k1 in {JOINT_K1_RANGE} with {JOINT_N_K_PER_DIM} steps")
    print(f"  k2 in {JOINT_K2_RANGE} with {JOINT_N_K_PER_DIM} steps")
    print(f"  k3 in {JOINT_K3_RANGE} with {JOINT_N_K_PER_DIM} steps")
    print(f"  a1 in {JOINT_A1_RANGE} with {JOINT_N_K_PER_DIM} steps")
    print(f"  a2 in {JOINT_A2_RANGE} with {JOINT_N_K_PER_DIM} steps")
    print("Early-time safety: Omega_BH(a_rec)/Omega_m(a_rec) < "
          f"{EARLY_BH_FRACTION_MAX:.1e}")
    print()

    best_all = joint_scan_all_params(
        z_sn, mu_sn, mu_err_sn,
        mean_desi, cov_desi,
        H0_range=JOINT_H0_RANGE,
        Om_range=JOINT_OM_RANGE,
    ...
    if best_all is None:
        print("No viable 7-parameter joint solution found in the scanned prior box.")
    else:
        (H0_7, Om_7,
         k1_7, k2_7, k3_7, a1_7, a2_7,
         M_7, H0rs_7,
         chi2_SN_7, chi2_DESI_7, chi2_tot_7) = best_all

        print("Best-fit 7-parameter joint SN+DESI solution (coarse global scan):")
        print(f"  H0      = {H0_7:.3f} km/s/Mpc")
        print(f"  Omega_m = {Om_7:.4f}")
        print(f"  k1      = {k1_7:.3f}   (early BH era)")
        print(f"  k2      = {k2_7:.3f}   (ignition phase)")
        print(f"  k3      = {k3_7:.3f}   (late-time)")
        print(f"  a1      = {a1_7:.4f}   (start of ignition, z ~ {1/a1_7 - 1:.1f})")
        print(f"  a2      = {a2_7:.4f}   (end of ignition,   z ~ {1/a2_7 - 1:.1f})")
        print(f"  M_best  = {M_7:.4f}   (SN absolute magnitude offset)")
        print(f"  H0*r_s  = {H0rs_7:.3f}")
        print(f"  chi^2_SN   = {chi2_SN_7:.2f}")
        print(f"  chi^2_DESI = {chi2_DESI_7:.2f}")
        print(f"  chi^2_tot  = {chi2_tot_7:.2f}")
        print()

        # Likelihood ratio vs ΛCDM-like logistic baseline
        if chi2_tot_LCDM is not None:
            delta_chi2_tot = chi2_tot_7 - chi2_tot_LCDM
            like_ratio = np.exp(-0.5 * delta_chi2_tot)
            print("=== Comparison vs ΛCDM-like logistic baseline (SN+DESI) ===")
            print(f"  chi^2_tot(ΛCDM-like) = {chi2_tot_LCDM:.2f}")
            print(f"  chi^2_tot(CCBH 7-par) = {chi2_tot_7:.2f}")
            print(f"  Δchi^2 = chi^2(CCBH) - chi^2(ΛCDM-like) = {delta_chi2_tot:.2f}")
            print(f"  Likelihood ratio L_CCBH / L_LCDM ≈ {like_ratio:.3e}")
            print("    (Values > 1 would favor CCBH, < 1 favor ΛCDM, within this coarse grid.)")
        else:
            print("Baseline ΛCDM-like total chi^2 not available; cannot form likelihood ratio.")
        print()

        print("Publishable-style parameter summary (coarse, grid-based):")
        print("  Model: 3-phase CCBH with cosmological coupling")
        print(f"  H0      = {H0_7:.3f} km/s/Mpc")
        print(f"  Omega_m = {Om_7:.4f}")
        print(f"  k1      = {k1_7:.3f}")
        print(f"  k2      = {k2_7:.3f}")
        ...
        print(f"  a1      = {a1_7:.4f}")
        print(f"  a2      = {a2_7:.4f}")
        print("  (Uncertainties require a dedicated MCMC / nested sampler;")
        print("   these values are best fits within the scanned grid.)")
        print()
